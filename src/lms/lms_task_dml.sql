...
select * from cars where cars.brand ilike 'Hyundai';
--#4 Отобразите все красные и синие автомобили
SELECT * FROM cars WHERE color LIKE 'Red' or color LIKE 'Blue';
...
select count(*) from cars where brand ilike 'Chevrolet';
...
select * from cars where brand like 'Audi' or cars.brand like 'Toyota' or cars.brand like 'Kia' or cars.brand like 'Ford';
...
select brand from cars where cars.brand ilike '_____';
--#12 Отобразите общую сумму всех автомобилей марки Mercedes-Benz
select sum(cars.price) from cars where brand like 'Mercedes-Benz';
--#13 Отобразите самую дорогую  и самую дешевую машину
select max(cars.price) as very_expensive ,min(cars.price) as least_expensive from cars;
...
select * from cars order by price desc offset 5 limit 10;
...
select cars.color , count(*) as naibolshi from cars group by color order by  naibolshi desc limit 1;
